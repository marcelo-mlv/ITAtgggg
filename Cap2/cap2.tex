...
This distinction is important for this thesis because the computational post-processing bottleneck aforementioned does not lie in the simulation execution itself, but in the processing pipeline from raw results to metrics and from metrics to batch-level analytical products.

\section{Distributed Processing}

Distributed processing is the use of multiple processing elements to perform computations cooperatively across a shared workload. In simulation-related research, this idea appears in both parallel and distributed simulation, where it may accelerate execution, distribute computational state, or support large-scale analytical post-processing \parencite{fujimoto2016pds}. In this thesis, distributed processing is not used to accelerate the simulator itself, but to accelerate the analytical treatment of the simulator's outputs.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\linewidth}{l >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
        ...
        \addlinespace
        Lagares et al. (2022) & Python API & Scalable, out-of-core analysis of large simulation results & Turbulent fluid flow / \gls{cfd} \\
        \addlinespace
        Morrissey et al. (2020) & Platform/API access layer for queries & Create a platform that enables in-situ processing and provide visualization of simulation results & Discrete Element Method \\
        \addlinespace
        ...
        \addlinespace
        \textbf{This TG proposal} & \textbf{ASA platform + Python API} & \textbf{Reduce data transfer, parallelize \gls{aa} Stage, and make batch-result evaluation more fast/scalable} & \textbf{Aerospace simulation} \\
        \bottomrule
    \end{tabularx}
\end{table}
