...
\subsection{Capacity-Based Planning}

% Definição de CBP
% Uso de CBP no MD
% Relevância no contetxo de defesa aérea

\subsection{The Value of Modeling and Simulation in Decision Making}

% Definição de M&S
% Relevância de M&S como auxílio para tomada de decisão

\subsection{ASA Project}

% Descrição do projeto ASA e como ele auxilia na tomada de decisão de autoridades
% Descrição do IEAv

\subsection{Sensitivity Analysis}

% Definição de SA
% Exemplificação de funcionamento (imagem)

\section{Motivation}

\subsection{Distributed Processing of Simulation Results}

% Explicação de como o Processamento Distribuído pode ser útil para processar os resultados de simulações
% Explicação das 2 fases de simulação nas quais PD pode ser útil (Execution Stage e Analysis & Aggregation Stage)
...
O capítulo 5 mostra as características e a operação do robô e do ambiente de simulação utilizados nos testes e experimentação da metodologia apresentada \cite{2008Accettura}.
